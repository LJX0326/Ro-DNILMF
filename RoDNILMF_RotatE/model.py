...
class KGEModel(nn.Module):
    ...
    def predict(model, X, from_idx=False):
        """
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        adversarial_temperature = args.adversarial_temperature/(1+tf.sigmoid(i))

        if args.negative_adversarial_sampling:
            #In self-adversarial sampling, we do not apply back-propagation on the sampling weight
            negative_score = (F.softmax(negative_score * adversarial_temperature, dim = 1).detach()
                              * tf.tanh(-negative_score)).sum(dim = 1)
        else:
            negative_score = tf.tanh(-negative_score).mean(dim = 1)

        positive_score = model(positive_sample)

        positive_score = tf.tanh(positive_score).squeeze(dim = 1)
                ...
                for test_dataset in test_dataset_list:
                    for positive_sample, negative_sample, filter_bias, mode in test_dataset:
                        ...
                        for i in range(0,batch_size):
                            for index,sc in enumerate(score_list[i]):
                                if sc > args.ebuselong:
                                    test_tri.append(positive_sample[i].numpy().tolist())
                                    new_index.append(index)
                                    score_sc.append(sc)
                                    mode_tri.append(mode)
                        for ind, tri in enumerate(test_tri):
                            if mode_tri[ind] == 'head-batch':
                                s.append(str(new_index[ind]) + ',' + str(tri[1]) + ',' + str(tri[2]))
                            if mode_tri[ind] == 'tail-batch':
                                s.append(str(tri[0]) + ',' + str(tri[1]) + ',' + str(new_index[ind]))
                        with open(r'save_path/score_text','w') as f1:
                            for i in s:
                                s2 = str(i)
                                s1 = ''.join(s2)
                                f1.write(s1 + '\n')
